...
export const DailyReportModal: React.FC<DailyReportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/70 backdrop-blur-md no-print animate-in fade-in duration-200">
      <div className="bg-slate-900 rounded-3xl shadow-2xl border border-slate-700/80 w-full max-w-4xl overflow-hidden flex flex-col max-h-[94vh] text-slate-100 ring-1 ring-slate-700/50">
        {/* Header - Glassmorphic Executive Bar */}
        <div className="px-5 py-4 border-b border-slate-800 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 text-white flex items-center justify-between shadow-inner">
          <div className="flex items-center gap-3.5">
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 text-white flex items-center justify-center shadow-lg shadow-indigo-600/30 shrink-0 ring-2 ring-indigo-400/30">
              <GraduationCap className="w-6 h-6 stroke-[2.2]" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="font-black text-sm sm:text-base text-white">
                  ارزیابی، تحلیل و گزارش عملکرد روزانه
                </h2>
                <span className="text-[10px] px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 font-black shrink-0">
                  ارزیابی هوشمند
                </span>
              </div>
              <p className="text-xs text-slate-400 mt-0.5 font-medium">
                ثبت پیشرفت وظایف، ارزیابی تراز زمانی و صدور گزارش تصویری رسمی
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-2 rounded-2xl text-slate-400 hover:text-white hover:bg-slate-800 transition-all cursor-pointer active:scale-90"
            title="بستن پنجره"
          >
        ...
        {/* TIER A (Top): Responsive, Fluid Day Selector Bar (شنبه تا جمعه) */}
        <div className="px-3 sm:px-5 py-2.5 bg-slate-950/90 border-b border-slate-800/90 select-none">
          <div className="grid grid-cols-7 gap-1.5 sm:gap-2 w-full max-w-full">
            {DAYS.map((d) => {
              ...
              return (
                <button
                  ...
                  className={`py-2 px-1 sm:px-2 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-1.5 cursor-pointer active:scale-95 truncate ${
                    isSelected
                      ? 'bg-gradient-to-r from-indigo-600 via-indigo-600 to-indigo-700 text-white shadow-lg shadow-indigo-950/80 ring-2 ring-indigo-400/60'
                      : 'bg-slate-900/90 hover:bg-slate-800 text-slate-300 border border-slate-800/80 hover:border-slate-700'
                  }`}
                  title={`${d.nameFa} (${toFaDigits(count)} فعالیت)`}
                >
                  <span
                    className="w-2 h-2 rounded-full shadow-xs shrink-0 hidden xs:inline-block"
                    style={{ backgroundColor: isSelected ? '#ffffff' : d.dotColor }}
                  />
                  <span className="truncate">{d.nameFa}</span>
                  <span
                    className={`text-[10px] px-1.5 py-0.2 rounded-md font-mono font-bold shrink-0 ${
                      isSelected
        ...
        {/* TIER B (Middle): Key Day Metrics (Start) & Format / Export Switcher Tabs (End) */}
        <div className="px-3 sm:px-5 py-2.5 bg-slate-900/95 border-b border-slate-800/80 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 backdrop-blur-md">
          {/* Start Side: Key Day Metrics Pills */}
          <div className="flex items-center flex-wrap gap-2 text-xs">
            <span
              className="flex items-center gap-1.5 font-bold bg-slate-950/70 text-slate-200 px-2.5 py-1.5 rounded-xl border border-slate-800"
              title="مجموع زمان برنامه‌ریزی‌شده برای این روز"
            >
              <Clock className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
              <span>مجموع: {toFaDigits(dayMetrics.totalHours)} ساعت</span>
            </span>

            <span
              className="flex items-center gap-1.5 font-bold bg-indigo-950/40 text-indigo-300 px-2.5 py-1.5 rounded-xl border border-indigo-800/50"
              title="ساعات کار عمیق و تمرکز بالا"
            >
            ...
            <span
              className={`flex items-center gap-1.5 font-bold px-2.5 py-1.5 rounded-xl border ${
                dayMetrics.completionRate >= 75
                  ? 'bg-emerald-950/40 text-emerald-300 border-emerald-700/50'
                  : 'bg-amber-950/40 text-amber-300 border-amber-800/50'
              }`}
              title="درصد پایبندی و تکمیل وظایف امروز"
            >
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
              <span>{toFaDigits(dayMetrics.completionRate)}٪ انجام شده</span>
            </span>

            <span className="hidden lg:inline-flex items-center gap-1 text-[11px] text-slate-400 font-medium pr-1">
              ({toFaDigits(dayMetrics.completedCount)} از {toFaDigits(dayMetrics.totalBlocks)} تسک)
            </span>
          </div>

          {/* End Side: Segmented Format / Export Switcher Tabs */}
          <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none shrink-0 p-1 bg-slate-950/80 rounded-2xl border border-slate-800/80">
            {/* Tab 0: Daily Audit & Report Card */}
            <button
              type="button"
              onClick={() => setActiveTab('audit')}
              className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-black rounded-xl transition-all cursor-pointer shrink-0 whitespace-nowrap ${
                activeTab === 'audit'
                  ? 'bg-emerald-600 text-white shadow-md shadow-emerald-950/50 ring-1 ring-emerald-400/50'
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-black rounded-xl transition-all cursor-pointer shrink-0 whitespace-nowrap ${
                activeTab === 'minimal'
                  ? 'bg-indigo-600 text-white shadow-md shadow-indigo-950/50 ring-1 ring-indigo-400/50'
                  : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900'
              }`}
            >
              <Sparkles className="w-3.5 h-3.5" />
              <span>خروجی گرافیکی</span>
            </button>
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-black rounded-xl transition-all cursor-pointer shrink-0 whitespace-nowrap ${
                activeTab === 'image'
                  ? 'bg-amber-600 text-white shadow-md shadow-amber-950/50 ring-1 ring-amber-400/50'
                  : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900'
              }`}
            >
        ...
        {/* TIER C (Bottom Canvas): Report Body & Rollover Engine */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-950/40">
          {/* TAB 0: INTERACTIVE DAILY AUDIT & REPORT CARD */}
          {activeTab === 'audit' && (
            <div className="space-y-4">
              {auditableBlocks.length === 0 ? (
                <div className="bg-slate-900 rounded-3xl border border-slate-800 p-8 text-center max-w-lg mx-auto shadow-xl">
                  <div className="w-14 h-14 rounded-2xl bg-emerald-500/20 text-emerald-400 flex items-center justify-center mx-auto mb-3.5 ring-1 ring-emerald-500/30">
                    <Coffee className="w-7 h-7" />
                  </div>
                  <h3 className="font-black text-slate-100 text-lg mb-1.5">
                    روز آزاد و استراحت برای {dayInfo.nameFa}
                  </h3>
                  <p className="text-xs text-slate-400 leading-relaxed">
                    در این روز هیچ فعالیت کاری یا آموزشی برای ارزیابی ثبت نشده است. می‌توانید با کلیک روی روزهای دیگر در نوار بالا، عملکرد سایر روزها را ارزیابی و ثبت کنید.
                  </p>
                </div>
              ) : !isAuditSubmitted ? (
                ...
                <div className="max-w-3xl mx-auto space-y-4">
                  {/* Top Control Bar */}
                  <div className="bg-slate-900/90 backdrop-blur-sm rounded-2xl border border-slate-800 p-4 shadow-md flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-black text-indigo-300 bg-indigo-950/80 px-2.5 py-1 rounded-lg border border-indigo-500/30">
                          {toFaDigits(auditableBlocks.length)} فعالیت کاری و آموزشی
                        </span>
                        <h3 className="font-black text-white text-sm">
                          ارزیابی عملکرد روز: {dayInfo.nameFa}
                        </h3>
                      </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <button
                        ...
                        className="text-xs font-black text-emerald-300 hover:text-emerald-200 bg-emerald-950/60 hover:bg-emerald-900/70 border border-emerald-500/40 px-3.5 py-2 rounded-xl transition-all cursor-pointer flex items-center gap-1.5 shadow-2xs active:scale-95"
                      ...
                      <button
                        ...
                        className="text-xs font-black text-indigo-300 hover:text-white bg-indigo-950/80 hover:bg-indigo-900 border border-indigo-500/40 px-3.5 py-2 rounded-xl transition-all cursor-pointer shadow-xs active:scale-95"
                      >
                        مشاهده کارنامه ←
                      </button>
                    </div>
                    ...
                    <div className="space-y-3.5">
                      {auditableBlocks.map((block) => {
                        ...
                        return (
                          <div
                            key={block.id}
                            className="bg-slate-900/90 rounded-2xl border border-slate-800 p-4 shadow-sm hover:border-slate-700 transition-all space-y-3"
                          >
                            {/* Block Header Info */}
                            <div className="flex items-start justify-between gap-3 border-b border-slate-800 pb-3">
                              <div className="flex items-center gap-3">
                                <div className="p-2 rounded-xl bg-slate-800 text-indigo-400 ring-1 ring-slate-700">
                              ...
                              <span className="text-xs font-mono font-bold bg-slate-950 text-slate-300 px-2.5 py-1 rounded-lg border border-slate-800">
                              ...
                              {[
                                ...
                                return (
                                  <button
                                    ...
                                    className={`py-2 px-2.5 rounded-xl border text-xs font-black transition-all cursor-pointer text-center active:scale-95 ${
                                      isSelected
                                        ? opt.id === 'none'
                                          ? 'bg-rose-600 text-white border-rose-500 shadow-md shadow-rose-950/60 ring-2 ring-rose-400/40'
                                          : 'bg-emerald-600 text-white border-emerald-500 shadow-md shadow-emerald-950/60 ring-2 ring-emerald-400/40'
                            ...
                            {resp.status === 'none' ? (
                              ...
                              <div className="bg-rose-950/40 border border-rose-500/30 rounded-xl p-3 space-y-2 text-rose-200">
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className="text-[11px] font-mono font-bold text-rose-300 bg-rose-950/80 px-2.5 py-0.5 rounded-md border border-rose-500/40">
                                    زمان واقعی: ۰ دقیقه
                                  </span>
                                </div>
                                    ...
                                    <button
                                      ...
                                      className={`text-xs font-bold px-3 py-1 rounded-lg border transition-all cursor-pointer ${
                                        resp.note === reason.label
                                          ...
                                          : 'bg-slate-800 text-rose-200 border-rose-900/60 hover:bg-rose-950/60'
                                      }`}
                                    >
                                      {reason.label}
                                    </button>
                              ...
                              <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-3 space-y-2.5">
                                <div className="flex flex-wrap items-center justify-between gap-2">
                                  <div className="flex items-center gap-2 text-xs text-slate-300">
                                    <span className="font-black text-white">زمان واقعی صرف‌شده:</span>
                                    <span className="font-mono font-black text-indigo-300 bg-indigo-950/80 px-2.5 py-0.5 rounded-md border border-indigo-500/30">
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    <button
                                      ...
                                      className="text-xs font-bold px-2.5 py-1 rounded-lg bg-slate-800 border border-slate-700 text-slate-200 hover:bg-slate-700"
                                    ...
                                    <button
                                      ...
                                      className="text-xs font-bold px-2 py-1 rounded-lg bg-slate-800 border border-slate-700 text-slate-200 hover:bg-slate-700"
                                      title="۱۵ دقیقه کمتر"
                                    >
                                      ۱۵-
                                    </button>
                                ...
                                <div className="flex items-center gap-2 pt-2 border-t border-slate-800">
                                  <span className="text-xs font-bold text-slate-400">کیفیت تمرکز:</span>
                                  {[
                                    { id: 'deep', label: '🔥 عمیق' },
                      ...
                      <div className="pt-2">
                        <button
                          type="button"
                          onClick={() => setIsAuditSubmitted(true)}
                          className="w-full py-3.5 rounded-2xl text-xs font-black text-white bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 shadow-md shadow-emerald-950/50 transition-all cursor-pointer flex items-center justify-center gap-2 ring-1 ring-emerald-400/40 active:scale-98"
                        >
                    ...
                    return (
                      <div className="bg-slate-900 rounded-3xl border border-slate-800 p-6 shadow-xl space-y-6">
                        ...
                        <div className="flex items-start justify-between border-b border-slate-800 pb-4">
                          <div className="flex items-center gap-3.5">
                            <div className="p-3 rounded-2xl bg-slate-800 text-indigo-400 ring-1 ring-slate-700">
                              <CategoryIcon category={currentBlock.category} className="w-5 h-5" />
                            </div>
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="text-xs font-black text-indigo-300 bg-indigo-950/80 px-2.5 py-0.5 rounded-lg border border-indigo-500/30">
                          ...
                          <span className="text-xs font-mono font-bold bg-slate-950 text-slate-300 px-3 py-1.5 rounded-xl border border-slate-800">
                        ...
                        <div>
                          <label className="block text-xs font-black text-slate-200 mb-2.5">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                            {[
                              ...
                              <button
                                ...
                                className={`p-3 rounded-2xl border text-xs font-black text-center transition-all cursor-pointer active:scale-95 ${
                        ...
                        {currentResp.status === 'none' ? (
                          ...
                          <div className="bg-rose-950/40 border border-rose-500/30 rounded-2xl p-4 space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-xs font-mono font-bold text-rose-300 bg-rose-950/80 px-2.5 py-1 rounded-lg border border-rose-500/40">
                          ...
                          <>
                            ...
                            <div>
                              <div className="flex items-center justify-between mb-2.5">
                                ...
                                <span className="text-xs font-mono font-black text-indigo-300 bg-indigo-950/80 px-3 py-1 rounded-lg border border-indigo-500/30">
                              ...
                              <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
                                {[15, 30, 45, 60, 90, 120, 150, 180, 210, 240, 300].map((dur) => (
                                  <button
                                    ...
                                    className={`text-xs font-mono font-bold px-3 py-1.5 rounded-xl border transition-all shrink-0 cursor-pointer ${
                              ...
                              <div className="flex items-center gap-2 mt-2.5">
                                <button
                                  ...
                                  className="text-xs px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold border border-slate-700 cursor-pointer"
                                >
                                  ۳۰ دقیقه کمتر -
                                </button>
                                <button
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-700 bg-slate-950 text-white placeholder-slate-500 focus:border-indigo-500 focus:outline-hidden"
                          />
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-slate-800">
                          <button
                            ...
                            className="px-4 py-2.5 rounded-xl text-xs font-bold text-slate-300 bg-slate-800 hover:bg-slate-700 transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                          >
                            ← فعالیت قبلی
                          </button>
                          ...
                          {auditStep < auditableBlocks.length - 1 ? (
                            <button
                              ...
                              className="px-5 py-2.5 rounded-xl text-xs font-black text-white bg-indigo-600 hover:bg-indigo-500 shadow-md transition-all cursor-pointer"
                            >
                              فعالیت بعدی →
                            </button>
                          ) : (
                            <button
                              type="button"
                              onClick={() => setIsAuditSubmitted(true)}
                              className="px-6 py-2.5 rounded-xl text-xs font-black text-white bg-emerald-600 hover:bg-emerald-500 shadow-md shadow-emerald-950/50 transition-all cursor-pointer flex items-center gap-2 ring-1 ring-emerald-400/40"
                            >
                              <Trophy className="w-4 h-4" />
