...
export const EmergencyRecoveryModal: React.FC<EmergencyRecoveryModalProps> = ({
  ...
  const handleCopyReport = () => {
    ...
    const text = `🌱 جاموندن از برنامه اینطوری: بازآرایی و تعهدنامه دوستانه ضدشکننده
روز: ${dayName}
علت رخداد: ${activeCause.label}
زمان بازآرایی: ${toFaDigits(lostMinutes)} دقیقه
ساعت آغاز تنظیم مجدد: ${toFaDigits(minutesToTimeString(currentMinutes))}
ساعات کار برنامه‌ریزی‌شده: ${toFaDigits(timeSummary.planned)} ساعت
ساعات بازآرایی‌شده امروز: ${toFaDigits(timeSummary.newScheduled)} ساعت
ساعات منتقل‌شده برای جبران: ${toFaDigits(timeSummary.debt)} ساعت

⚡ گام پیشنهادی برای بازگشت به تمرکز:
...
«۱ همیشه از ۰ بزرگ‌تر است - حرکت از همین لحظه پیروزی است!»`;
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base sm:text-lg font-black tracking-tight">
                  جاموندن از برنامه اینطوری (تنظیم مجدد و جبران مهربانانه)
                </h2>
                <span className="text-[10px] bg-emerald-600 text-white font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                  Anti-Fragile Compassion
                </span>
              </div>
              <p className="text-xs text-slate-300 mt-0.5 font-medium">
                وقتی خستگی، گوشی یا بی‌نظمی پیش آمد، بدون سرزنش خود، برنامه را هوشمندانه از همین لحظه ادامه بده!
              </p>
            </div>
          </div>
